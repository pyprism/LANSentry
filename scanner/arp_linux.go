...
// pingHost sends a single ping to the host using fping for speed.
func pingHost(ip string) {
	// Try fping first (faster), fall back to ping
...
func (s *ARPScanner) readArpTable() ([]device.Device, error) {
	// Try /proc/net/arp first (faster on Linux)
	devices, err := s.readProcArp()
	if err == nil {
		return devices, nil
	}
...
// readProcArp reads the ARP table from /proc/net/arp.
func (s *ARPScanner) readProcArp() ([]device.Device, error) {
	...
	for i, line := range lines {
		...
		if len(fields) >= 6 {
			...
			if s.interfaceName != "" && iface != s.interfaceName {
				continue
			}
			...
			// Skip multicast IPs and MACs
			if isMulticastIP(ip) || isMulticastMAC(mac) {
				continue
			}
...
func parseArpOutput(output, interfaceName string) ([]device.Device, error) {
	...
	//   myhost (192.168.1.5) at 00:11:22:33:44:55 [ether] on eth0
